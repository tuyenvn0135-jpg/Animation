...
if ($action == 'add' && $id > 0) {
    ...
    header("Location: MID/cart.php"); exit;
}
    ...
    <?php if(!empty($_SESSION['cart'])): ?>
        <div class="bg-white rounded-xl border shadow-sm overflow-hidden text-xs">
                ...
                <tbody class="divide-y">
                    ...
                    <tr>
                        ...
                        <td class="p-4 text-center">
                            <a href="MID/cart.php?action=delete&id=<?php echo $p_id; ?>" class="text-red-500 hover:underline">Gỡ khỏi giỏ</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            ...
            <div class="p-4 bg-gray-50 flex justify-between items-center border-t">
                ...
                <a href="MID/checkout.php" class="bg-amber-500 hover:bg-amber-600 text-slate-950 font-black px-6 py-2.5 rounded-lg text-xs uppercase tracking-wider transition">Tiến hành đặt hàng →</a>
            </div>
        </div>
    <?php else: ?>
