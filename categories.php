...
<div class="container mx-auto px-4 py-10 max-w-7xl">
    ...
    <form action="categories.php" method="GET" class="bg-white p-4 rounded-xl border shadow-sm mb-8 grid grid-cols-1 md:grid-cols-4 gap-4" data-aos="fade-down">
    ...
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <aside class="w-full lg:w-1/4 bg-white p-6 rounded-xl shadow-sm h-fit border border-gray-100 text-xs font-semibold">
            ...
            <form action="categories.php" method="GET" class="space-y-6">
                ...
                <?php if($products && $products->num_rows > 0): ?>
                    <?php while($row = $products->fetch_assoc()): ?>
                        <div class="bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition flex flex-col justify-between text-xs font-medium" data-aos="fade-up">
                            ...
                            <div class="p-4 pt-0">
                                <a href="cart.php?action=add&id=<?php echo $row['id']; ?>" class="w-full block text-center bg-[#0A2540] text-white py-2 rounded text-[11px] font-bold uppercase hover:bg-blue-900 transition shadow-sm">🛒 Thêm giỏ hàng</a>
                            </div>
                        </div>
                    <?php endwhile; ?>
                <?php else: ?>
